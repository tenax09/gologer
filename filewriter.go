...
func (adapterFile *AdapterFile) initFile() error {
	...
	ok, _ := NewFile().PathExists(adapterFile.config.Filename)
	if ok == false {
		err := NewFile().CreateFile(adapterFile.config.Filename)
		if err != nil {
			return err
		}
	}
	...
	nowLines, err := NewFile().GetFileLines(adapterFile.config.Filename)
	if err != nil {
		return err
	}
